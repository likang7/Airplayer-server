...
ServerMediaSession*
MyRTSPServer::lookupServerMediaSession(char const* streamName) {
...
CONVERT:
        ...
        const string destpath = string(fileName) + ".___DIRECT.ts";
        ...
        if(res[i].streamType == Video)
        {
            if(res[i].content == "h264")
            {
                ...
                const string cmd = string("echo N | ffmpeg") + " -i \"" + srcFile + \
                                   "\" -acodec copy -vcodec copy -threads 4 -vbsf h264_mp4toannexb \"" + destpath + "\"";
